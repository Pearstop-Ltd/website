...
/** Decorative dash directly under every case page H1: 72x4px, 2px radius,
 * aria-hidden. Legacy-system equivalent of components/site/HeadlineAccent. */
export function CaseHeadlineAccent() {
  return <span aria-hidden="true" className={styles.accent} />;
}

/** Hero grid used by every case detail page: title/lead on the left, an
 * optional aside (fact sheet or bespoke visual) on the right. `equal` uses
 * a 1:1 column split (centered) instead of the default 1.55:1 (top-aligned)
 * split — matches the two hero shapes actually in use across case pages. */
export function CaseHero({ children, aside, equal }: { children: ReactNode; aside?: ReactNode; equal?: boolean }) {
  if (!aside) return <div style={{ display: "flex", flexDirection: "column", gap: 22, maxWidth: 780 }}>{children}</div>;
  return (
    <div className={equal ? styles.heroEqual : styles.hero}>
  ...
  return (
    <section style={{ background: "#fff", padding: "0 0 96px" }}>
      <div className="container">
        <div className={styles.resultsBand} style={{ "--cols": stats.length } as CSSProperties}>
          {stats.map((s) => (
            <div key={s.label} className={styles.resultsCell}>
              <span className={styles.resultsValue}>{s.value}</span>
              <span className={styles.resultsLabel}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
...
export function CaseProcessDiagram({ inputs, steps, outputs }: {
  ...
  return (
    <div className={styles.processDiagram}>
      ...
      <CaseArrow />
      <div className={styles.processOutputs}>
        {outputs.map((o) => (
...
function CaseArrow() {
  return (
    <svg width="36" height="12" viewBox="0 0 36 12" fill="none" aria-hidden="true" className={styles.arrow}>
...
export function CaseStepCards({ steps }: { steps: { title: string; copy: string }[] }) {
  return (
    <div className={styles.stepCards} style={{ "--cols": steps.length } as CSSProperties}>
      {steps.map((s, i) => (
...
export function CaseDataTable({ columns, rows, footnote }: {
  ...
  const cols = { "--cols": columns.length } as CSSProperties;
  return (
    <div>
      <div className={styles.dataTableScroll}>
        <div style={{ border: "1px solid var(--border)", borderRadius: 20, overflow: "hidden" }}>
          <div className={styles.dataTableGrid} style={{ ...cols, background: "var(--bg-soft)", padding: "14px 28px", fontSize: 12, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--muted)" }}>
            {columns.map((c) => <span key={c}>{c}</span>)}
          </div>
          {rows.map((row, i) => (
            <div key={i} className={styles.dataTableGrid} style={{ ...cols, padding: "16px 28px", borderTop: "1px solid var(--border)", alignItems: "center", fontSize: 15 }}>
 ...
 * everywhere they're quoted) or is anonymous (`role` only) — anonymized
 * pattern cases never get a name or a photo. */
type QuoteAttribution = { personId: PersonId; role?: never } | { personId?: never; role: string };

export function CaseQuoteBig({ quote, ...attribution }: { quote: string } & QuoteAttribution) {
  ...
  return (
    <figure className={styles.quoteBig} style={{ margin: 0, background: "var(--primary-dark)", borderRadius: 24, display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <figcaption style={{ display: "flex", gap: 14, alignItems: "center" }}>
        ...
        <span style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <span style={{ fontSize: 13, color: "var(--blue-soft)" }}>{displayRole}</span>
        </span>
      </figcaption>
    </figure>
  );
...
export function CaseQuoteCard({ quote, bg = "var(--primary-dark)", ...attribution }: { quote: string; bg?: string } & QuoteAttribution) {
  const person = attribution.personId ? getPerson(attribution.personId) : undefined;
  const displayRole = person ? `${person.role}, ${person.company}` : attribution.role;
  return (
    <figure className={styles.quoteCard} style={{ margin: 0, border: "1px solid var(--border)", borderRadius: 24 }}>
      {person?.headshot ? (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <figcaption style={{ fontSize: 14, color: "var(--muted)" }}>
          {person ? <strong style={{ color: "var(--navy)", fontWeight: 600 }}>{person.name}</strong> : null}
          {person ? " · " : null}
          {displayRole}
        </figcaption>
      </div>
    ...
    <section style={{ background: "var(--bg-soft)", padding: "80px 0" }}>
      <div className="container">
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          ...
          <div className={styles.moreLinksGrid}>
            {items.map((item) => (
      ...
      <div className="container">
        <div className={styles.closingTop}>
          ...
          <div className={styles.closingActions}>
            {sample ? (
              <SampleRequestModal label={ctaLabel} className={primaryButtonClassName} />
            ) : ctaHref === siteConfig.calendly ? (
              <CalendlyButton label={ctaLabel} className={primaryButtonClassName} style={primaryButtonStyle} />
            ) : (
