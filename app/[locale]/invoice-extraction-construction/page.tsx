...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "InvoiceExtractionConstruction" });
  return {
    title: t("meta.title"),
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 3, name: "Invoice Extraction for Construction", item: PAGE_URL }
  ]
};

export default async function InvoiceExtractionConstructionPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("InvoiceExtractionConstruction");
