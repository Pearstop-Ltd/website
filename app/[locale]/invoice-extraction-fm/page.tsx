...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "InvoiceExtractionFM" });
  return {
    title: t("meta.title"),
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 3, name: "Invoice Extraction for Facilities Management", item: PAGE_URL }
  ]
};

export default async function InvoiceExtractionFMPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("InvoiceExtractionFM");
