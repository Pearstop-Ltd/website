...
export default async function SpendCubePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const prefix = locale === "en" ? "" : `/${locale}`;
  const t = await getTranslations("SpendCube");
          ...
          <div className="row" style={{ gap: "2rem", flexWrap: "wrap" }}>
            <div className="col-md-6">
              <div className="quote-card" style={{ height: "100%" }}>
                ...
                <Link className="bene-link" href={`${prefix}/blog/spend-cube-execution-framework`}>
