...
export default async function AiReadinessPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations("AiReadiness");
  ...
  return (
    <>
          ...
          <div className="row" style={{ alignItems: "center", gap: "3rem", flexWrap: "wrap" }}>
            ...
            <div className="col-md-5" style={{ marginLeft: "auto" }}>
              <div className="quote-card">
                <div className="story-label">{t("problem.commonThread.label")}</div>
                <p className="light-copy">
                  {t("problem.commonThread.copy")}
                </p>
              </div>
            </div>
          </div>
      ...
      <section id="how-it-works" className="section-soft">
        <div className="container">
          <SectionTitle eyebrow={t("problem.eyebrow")} title={t("howItWorks.title")} />
      ...
      <section>
        <div className="container">
          <SectionTitle title={t("benefits.b1.title")} />
          <div className="bene-cards">
            <article className="ben-card">
              <div className="ben-icon">↗</div>
              <h3>{t("benefits.b1.title")}</h3>
              <p>{t("benefits.b1.copy")}</p>
            </article>
            <article className="ben-card">
              <div className="ben-icon">≡</div>
              <h3>{t("benefits.b2.title")}</h3>
              <p>{t("benefits.b2.copy")}</p>
            </article>
            <article className="ben-card">
              <div className="ben-icon">⚡</div>
              <h3>{t("benefits.b3.title")}</h3>
              <p>{t("benefits.b3.copy")}</p>
            </article>
          </div>
        </div>
      </section>
      ...
      <CTABand
        ...
        actions={[
          { label: t("cta.button"), href: siteConfig.calendly, variant: "primary", external: true },
          { label: t("cta.fabricLink"), href: `${prefix}/fabric`, variant: "secondary" }
        ]}
      />
    </>
  );
